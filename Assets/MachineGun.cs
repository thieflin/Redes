...
public class MachineGun : Weapon
{
    ...
    {
        var newBullet = Instantiate(bulletPrefab, fireTransforms[count].transform.position, transform.rotation);

        if (targetEnemy)
        {
            newBullet.GetComponent<Bullet>().SetBullet(10);
            newBullet.GetComponent<Bullet>().target = targetEnemy.transform;
